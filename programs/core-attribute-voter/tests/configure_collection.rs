...
async fn test_configure_collection() -> Result<(), TransportError> {
    ...
    let collection_config_cookie = core_voter_test
        .with_collection(
            ...
            Some(ConfigureCollectionArgs { max_weight: 1, ..Default::default() }),
        )
        .await?;
...
async fn test_configure_multiple_collections() -> Result<(), TransportError> {
    ...
    core_voter_test
        .with_collection(
            ...
            &collection_cookie2,
            &max_voter_weight_record_cookie,
            Some(ConfigureCollectionArgs { max_weight: 2, ..Default::default() }),
        )
        .await?;
...
async fn test_configure_collection_with_invalid_max_voter_weight_realm_error(
    ...
    assert_nft_voter_err(err, CoreNftAttributeVoterError::InvalidMaxVoterWeightRecordRealm);
...
async fn test_configure_collection_with_invalid_max_voter_weight_mint_error(
    ...
    assert_nft_voter_err(err, CoreNftAttributeVoterError::InvalidMaxVoterWeightRecordMint);

    Ok(())
}

#[tokio::test]
async fn test_configure_collection_with_empty_weight_attribute_key_error(
) -> Result<(), TransportError> {
    // Arrange
    let mut core_voter_test = CoreVoterTest::start_new().await;
    ...
    let err = core_voter_test
        .with_collection(
            ...
            Some(ConfigureCollectionArgs {
                weight_attribute_key: "".to_string(),
                ..Default::default()
            }),
        )
        .await
...
async fn test_configure_collection_with_too_long_weight_attribute_key_error(
    ...
    // Act — key with 33 chars (exceeds 32 limit)
    let err = core_voter_test
        .with_collection(
            &registrar_cookie,
            &collection_cookie,
            &max_voter_weight_record_cookie,
            Some(ConfigureCollectionArgs {
                weight_attribute_key: "abcdefghijklmnopqrstuvwxyz1234567".to_string(),
                ..Default::default()
            }),
        )
        .await
