...
pub fn relinquish_nft_vote(ctx: Context<RelinquishNftVote>) -> Result<()> {
    ...
    if proposal.state == ProposalState::Voting {
        ...
        require!(
            ...
            CoreNftAttributeVoterError::InvalidVoteRecordForNftVoteRecord
        );
        ...
        if !vote_record_info.data_is_empty() {
            let data = vote_record_info
                .try_borrow_data()
                .map_err(|_| CoreNftAttributeVoterError::VoteRecordBorrowFailed)?;
            require!(
                data[0] == 0,
                CoreNftAttributeVoterError::VoteRecordMustBeWithdrawn
            );
        }
    }
    ...
    if voter_weight_record.voter_weight_expiry >= Some(Clock::get()?.slot) {
        return err!(CoreNftAttributeVoterError::VoterWeightRecordMustBeExpired);
    }
